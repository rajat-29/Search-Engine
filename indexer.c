...
int main(int argc,char* argv[]) {
    ...
    for(int i=0;i<4;i++)
    {
        sprintf(File_Name,"%d",file_no);  // name of the file
        strcat(File_Name, ".txt");  // adding txt format
        file_no++;                  // increase file no
        char *fileData = (char*)malloc(sizeof(char) * fileSize);  // file size
        fileData = convertDataInStr(File_Name);  // convert data into string

    	find_Title(fileData,File_Name);  // search tile tag
        find_Meta(fileData,File_Name);  // search the meta tag
    	find_Heading(fileData,File_Name);  // search heading tag
